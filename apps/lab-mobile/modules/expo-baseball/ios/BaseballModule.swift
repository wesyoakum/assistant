...
public final class BaseballModule: Module {
  ...
  public func definition() -> ModuleDefinition {
    ...
    OnCreate {
      do {
        let candidates: [URL?] = [
          Bundle.main.url(forResource: "BaseballDetector", withExtension: "mlmodelc"),
          Bundle.main.url(forResource: "BaseballDetector", withExtension: "mlmodelc", subdirectory: "ExpoBaseballModels.bundle"),
          {
            if let bundleURL = Bundle.main.url(forResource: "ExpoBaseballModels", withExtension: "bundle"),
               let b = Bundle(url: bundleURL) {
              return b.url(forResource: "BaseballDetector", withExtension: "mlmodelc")
            }
            return nil
          }(),
        ]
        guard let modelURL = candidates.compactMap({ $0 }).first else {
          self.loadError = "BaseballDetector.mlmodelc not found in app bundle"
          return
        }
    ...
    AsyncFunction("detect") { (uri: String, opts: [String: Any]) -> [String: Any] in
      guard let vModel = self.visionModel else {
        throw BaseballError.notReady(self.loadError ?? "model not loaded")
      }
      guard let image = loadCGImage(uri: uri) else {
        throw BaseballError.imageLoadFailed
      }
...
enum BaseballError: Error, LocalizedError {
  case notReady(String)
  case imageLoadFailed
  var errorDescription: String? {
    switch self {
    case .notReady(let why): return "Baseball detector not ready: \(why)"
